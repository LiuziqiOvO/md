...
<<<<<<< HEAD
对于数据处理，OmniLib实现了一个基于模型的引擎，可以动态决定（B）是否将处理操作（例如，读取CRC后写入）卸载到主机（C）或设备（D）。最后，OmniLib还支持直接使用CXL.mem，通过将数据和命令直接复制到DevCache，避免排队延迟和数据复制。
=======
...
<<<<<<< HEAD
- 新写入的：写入HostCache，合并写入（2MB，与Cache Page粒度不符）
=======
...
<<<<<<< HEAD
在 OmniCache 中（如图3所示），我们通过一个结合了 I/O 和数据处理的操作（“read-cal_- distance-nearestK ”）来处理 KNN 操作。此操作读取数据范围，计算距离并选择 K 个最近的点。中间结果（如计算出的距离）存储在处理缓冲区中。OmniCache 同时在主机和设备上并发执行“读取-计算距离-K最近邻”操作，利用协同处理。
=======
