...
Key-value stores are widely-used storage engines for data management, with Log-Structured Merge (LSM) trees being the de-facto standard data structure for managing key-value pairs. Prominent key-value stores such as RocksDB, LevelDB, BigTable, Cassandra, and HBase leverage LSM-trees for their high write performance and efficient management of key-value pairs. LSM-trees store incoming key-value pairs in an in-memory structure called MemTable. When the MemTable reaches a certain size, it is flushed to storage as a Sorted String Table (SST) file. SST files are organized in a multi-level structure to optimize read performance. When a level exceeds its space limit, a compaction process merges SST files from that level with those in the next level. Both flush and compaction operations occur in the background, and SST files are written sequentially to maximize storage device performance.

>键值存储是广泛使用的数据管理存储引擎，而日志结构合并（LSM）树是管理键值对的事实标准数据结构。RocksDB、LevelDB、BigTable、Cassandra 和 HBase 等著名的键值存储利用 LSM 树实现了高写入性能和键值对的高效管理。LSM 树将输入的键值对存储在名为 MemTable 的内存结构中。当 MemTable 达到一定大小时，就会以排序字符串表（SST）文件的形式刷新存储。SST 文件以多级结构组织，以优化读取性能。当某一级超过空间限制时，压缩进程会将该级的 SST 文件与下一级的文件合并。刷新和压缩操作都在后台进行，SST 文件按顺序写入，以最大限度地提高存储设备性能。

The use of volatile memory for MemTables means data can be lost in the event of a power failure. To address this, a write-ahead log (WAL) is maintained in persistent storage. Each put request is first recorded in the WAL before being buffered in memory, enabling data recovery even after a failure. However, frequent small writes generated by a stream of put requests can reduce write throughput and introduce scalability bottlenecks. In multi-threaded environments, simultaneous WAL record writes can cause lock contention, leading to write latency spikes.

A common solution to this problem is to merge small writes and write them sequentially in bulk, as seen in RocksDB’s batch-group writes. This technique selects a leader thread among multiple worker threads with pending put requests, collects all records, and writes them at once. However, this can exacerbate tail latency problems, as the waiting time for individual writes can increase during batch collection and writing, especially for larger records.

> MemTables 使用易失性内存意味着一旦断电，数据就会丢失。为了解决这个问题，我们在持久存储中维护了一个前置写日志（WAL）。每个写入请求首先记录在 WAL 中，然后再缓冲到内存中，这样即使发生故障也能恢复数据。然而，频繁的小规模写入会降低写入吞吐量，并带来可扩展性瓶颈。在多线程环境中，同时写入 WAL 记录会造成锁竞争，从而导致写入延迟峰值。
>
> 解决这一问题的常见方法是合并小规模写入，并按顺序批量写入，如 RocksDB 的批量分组写入。这种技术会在多个有挂起写入请求的工作线程中选择一个领导线程，收集所有记录并一次性写入。不过，这可能会加剧尾部延迟问题，因为在批量收集和写入过程中，单个写入的等待时间会增加，尤其是对于较大的记录。

The introduction of Zoned Namespace (ZNS) SSDs addresses these challenges by dividing storage into zones that only allow sequential writes, simplifying the flash translation layer (FTL) and reducing garbage collection overhead. The zoned structure, proven in Shingled Magnetic Recording (SMR) HDDs, offers similar benefits for NAND flash media. ZNS SSDs have shown potential for LSM-tree-based key-value stores by supporting streams of sequential writes, as demonstrated by ZenFS, which uses a lightweight file structure tailored for ZNS SSDs. However, ZenFS only provides a zoned interface plugin, leaving the batch-group write problem unresolved. Solutions like SpanDB’s lock-free LBA allocation for parallel writes are difficult to apply due to ZNS constraints, requiring additional serialization to ensure write sequence consistency.

To improve tail latency for LSM trees on ZNS SSDs, we explore the zone append command, recently added to ZNS SSD specifications. Conventional NVMe interfaces do not guarantee write order among queued requests, complicating efforts to increase queue depth without impacting write throughput. The zone append command, specifying data and a designated zone (but not a specific LBA), allows the device to determine the exact LBA location and returns the starting location of the appended data to the host. This enables efficient synchronization within the ZNS SSD, lifting host-side queue depth restrictions and significantly improving write throughput and tail latency.

> 分区命名空间（ZNS）固态硬盘的推出解决了这些难题，它将存储划分为只允许顺序写入的区域，简化了闪存转换层（FTL）并减少了垃圾收集开销。分区结构已在带状磁记录（SMR）硬盘中得到验证，并为 NAND 闪存介质提供了类似的优势。ZenFS 采用专为 ZNS SSD 量身定制的轻量级文件结构，通过支持连续写入流，ZNS SSD 已显示出基于 LSM 树的键值存储的潜力。不过，ZenFS 只提供了一个分区接口插件，批量群组写入问题仍未解决。由于 ZNS 的限制，SpanDB 为并行写入提供的无锁 LBA 分配等解决方案很难应用，需要额外的序列化来确保写入序列的一致性。
>
> 为了改善 ZNS SSD 上 LSM 树的尾部延迟，我们探索了最近添加到 ZNS SSD 规范中的区域附加命令。传统的 NVMe 接口不能保证队列请求之间的写入顺序，这使得在不影响写入吞吐量的情况下增加队列深度的工作变得更加复杂。指定数据和指定区域（但不是特定 LBA）的区域附加命令允许设备确定准确的 LBA 位置，并将附加数据的起始位置返回给主机。这就实现了 ZNS SSD 内部的高效同步，解除了主机侧队列深度限制，显著提高了写入吞吐量和尾部延迟。

In ZAFS, we designed a new type of WAL for ZNS SSDs—Append-aware WAL. This WAL optimization is not limited to LSM-Tree-based storage engines. Zone-Friendly WAL can be beneficial for other databases using zone semantics for WAL storage, such as SQLite and HBase. However, these databases do not sufficiently support ZNS SSDs, so we will limit our discussion to LSM-tree storage engines on ZNS, using ZenFS as a reference model to explain why and how we modified the existing WAL design for ZNS. Subsequently, we discuss the integration of Zone-Friendly WAL in our ZAFS within the context of RocksDB and compare it with ZenFS. 
...
WALs' write operations are very frequent, and the storage devices' limitation on small writes leads to performance bottlenecks. However, read operations occur infrequently, typically during database recovery after an unexpected shutdown. Thus, we believe it is acceptable to trade off read performance for enhanced write performance. Our design goal is to improve WAL write performance and concurrency, fully utilizing the peak performance of zone appends. By eliminating the performance bottleneck caused by serial WAL writes, we aim to enhance the efficiency of the storage engine's Put operations.
...
>在当今数据管理中，键值存储引擎被广泛应用，而LSM-tree作为事实上的标准数据结构，被用于诸如RocksDB、LevelDB、BigTable、Cassandra和HBase等流行的键值存储中。LSM-tree通过在内存结构MemTable中快速存储键值对以提供高效的写性能。当MemTable达到一定大小时，会被排序、合并并存储为有序字符串表（SST）文件。SST文件以多级结构排列，以提供优异的读取性能。当每一级达到空间限制时，会选择一个受害的SST文件并与下一层级的SST文件合并（压缩）。为解决MemTable使用易失性内存导致的数据一致性问题，LSM-tree维护了一个写前日志（WAL）以在持久存储中记录写请求。然而，这种结构会产生频繁的小写操作，导致写吞吐量降低并引发可扩展性瓶颈。

The use of volatile memory for MemTables means data can be lost in the event of a power failure. To address this, a write-ahead log (WAL) is maintained in persistent storage. Each put request is first recorded in the WAL before being buffered in memory, enabling data recovery even after a failure. However, frequent small writes generated by a stream of put requests can reduce write throughput and introduce scalability bottlenecks. In multi-threaded environments, simultaneous WAL record writes can cause lock contention, leading to write latency spikes.

>Zoned Namespace (ZNS) SSDs 引入了一种将存储空间划分为只能顺序写入的区域（zones）的新方法，这简化了满足NAND闪存介质擦除前写入约束的闪存转换层（FTL）。这一分区结构显著减少了SSD中的垃圾回收开销。研究者们发现ZNS SSD作为LSM树的后备存储具有潜在的适应性，例如ZenFS提议将ZNS SSD用作RocksDB的存储介质。然而，现有解决方案如批处理组写入技术并未解决尾延迟问题。	
...
1. WALs are write-intensive and primarily issue small writes.
2. WALs are read only during database recovery.
3. WALs are usually small (i.e., 64 MiB).
...
Building on these insights, we propose ZAFS (Zone-Append File System), a novel key-value store that enhances the write-ahead log using the zone append command for LSM trees on ZNS SSDs. We integrate Intel SPDK, a lightweight user-level NVMe driver, with RocksDB on ZenFS, establishing a robust baseline. Our evaluations using db_bench microbenchmarks and Facebook MixGraph benchmarks demonstrate that ZAFS reduces tail latency by up to 3.02× for db_bench and 4.73× for MixGraph, while also improving query throughput (QPS) by 5.9% on average and up to 11.7% at maximum by eliminating batch-group write overhead on the host.
