...
> 分区命名空间（ZNS）固态硬盘采用基于分区的管理，在分区内执行顺序写入，有效解决了传统固态硬盘中写入放大和垃圾收集开销的关键难题。
> 与传统固态硬盘相比，ZNS 固态硬盘具有更高的吞吐量、更低更稳定的延迟以及更长的使用寿命。
> 然而，大多数现有研究都是基于模拟器实现的，这可能无法反映真实世界的应用场景；此外，调整应用程序以利用 ZNS 的硬件特性至关重要，但现有的调整未能充分发挥 ZNS 的性能。
>
> 我们的工作对 ZNS 模拟器和真实 ZNS 设备进行了比较评估，并对日志结构合并树和 RocksDB 存储引擎进行了深入分析。我们提出了一种考虑到区域追加的追加感知前写日志（A2-WAL），并实现了基于 SPDK 的 ZNS 专用文件系统 ZAFS（Zone-Append File System）。通过利用SPDK技术，我们利用区域追加命令和A2-WAL的异步写入机制，有效解决了RocksDB的串行先写日志在高写入场景下造成的性能瓶颈问题。
>
> 通过实验，我们获得了 ZNS SSD 硬件测试数据，并探索了 ZNS SSD 的设计原理和参数选择，评估和验证了 ZAFS 的有效性。结果表明，与 ZenFS 相比，ZAFS 的吞吐量提高了 1.9-3.5 倍。此外，虽然 A2-WAL 增加了文件大小，但在灾难恢复场景中，数据恢复时间最多可缩短 6.7 倍。
>
> 分区命名空间固态硬盘、日志结构合并树、先写日志、键值存储、文件系统
>

Zoned Namespace (ZNS) SSD employ zone-based management and enforce sequential writes within zones, effectively addressing the critical challenges of write amplification and garbage collection overhead in traditional SSDs. 
Compared to traditional SSDs, ZNS SSDs exhibit higher throughput, lower and more stable latency, and longer lifetimes. 
However, most existing works are based on simulator implementations, which may not reflect real-world scenarios; furthermore, adapting applications to utilize ZNS's hardware features is essential, but existing adaptations fail to fully exploit ZNS's performance.

Our work presents a comparative evaluation of ZNS simulators and real ZNS devices, combined with an in-depth analysis of log-structured merge trees and the RocksDB storage engine. We propose an append-aware write-ahead log (A2-WAL) that takes into account zone appending, and implement ZAFS (Zone-Append File System), a ZNS-specific file system based on SPDK. By leveraging SPDK technology, we utilize zone appending commands and A2-WAL's asynchronous writing mechanism to effectively address the performance bottleneck issue caused by RocksDB's serial write-ahead logging in high-write scenarios.

Through experiments, we obtain ZNS SSD hardware test data and explore the design principles and parameter selection of ZNS SSDs, evaluating and validating the effectiveness of ZAFS. The results show that, compared to ZenFS, ZAFS achieves a throughput improvement of 1.9-3.5 times. Moreover, although A2-WAL increases file size, the data recovery time can be reduced by up to 6.7 times in disaster recovery scenarios.



Zoned Namespace SSD, Log-Structured Merge Tree, Write-Ahead Log, Key-Value Storage, File System

# 1. Introduction

Key-value stores are widely-used storage engines for data management, with Log-Structured Merge (LSM) trees being the de-facto standard data structure for managing key-value pairs. Prominent key-value stores such as RocksDB, LevelDB, BigTable, Cassandra, and HBase leverage LSM-trees for their high write performance and efficient management of key-value pairs.

> 键值存储是广泛使用的数据管理存储引擎，而日志结构合并（LSM）树是管理键值对的事实标准数据结构。RocksDB、LevelDB、BigTable、Cassandra 和 HBase 等著名的键值存储利用 LSM 树实现了高写入性能和键值对的高效管理。

LSM-tree converts random writes into sequential writes, greatly improving write performance. LSM-trees store incoming key-value pairs in an in-memory structure called the **MemTable**. When the MemTable reaches a predefined size, it is converted to an **Immutable MemTable** that is read-only and then **flushed** to storage as a Sorted String Table (SST) file. The use of MemTable and Immutable MemTable facilitates append-only writes, enhancing performance.

> LSM 树将随机写入转换为顺序写入，大大提高了写入性能。LSM 树将输入的键值对存储在一个名为 **MemTable** 的内存结构中。当 MemTable 达到预定义的大小时，它会被转换为只读的不可变 MemTable，然后作为排序字符串表（SST）文件**刷新**存储。MemTable 和不可变 MemTable 的使用有助于只写入追加内容，从而提高性能。

SST files are organized in a multi-level structure to optimize read performance. Each level consists of several SSTables, with size limits that grow exponentially across levels. When a level exceeds its space limit, a compaction process merges SSTables from that level with those in the subsequent level, thereby maintaining efficient storage utilization.

> SST 文件以多级结构组织，以优化读取性能。每一级由多个 SSTables 组成，其大小限制在各级之间呈指数增长。当某一级超过空间限制时，压缩进程会将该级的 SSTables 与下一级的 SSTables 合并，从而保持高效的存储利用率。

Both flush and compaction operations occur in the background, and SST files are written sequentially to maximize storage device performance. This ensures that key-value stores utilizing LSM-trees, such as BigTable, LevelDB, and RocksDB, can handle large volumes of data efficiently and maintain high performance across various storage environments.

> 刷新和压缩操作都在后台进行，SST 文件按顺序写入，以最大限度地提高存储设备性能。这确保了使用 LSM 树的键值存储（如 BigTable、LevelDB 和 RocksDB）能够高效处理大量数据，并在各种存储环境中保持高性能。

The use of volatile memory for MemTables means data will be lost in the event of a power failure or system crash. To address this,  the LSM tree writes PUT operations to a Write-Ahead Log (WAL) in persistent storage. The WAL records all changes to key-value pairs over time. When the key-value store restarts, the LSM tree restores its state using a process called WAL recovery. WAL recovery sequentially reads all WAL data and reapplies it to the MemTable. Data must be applied in sequence because only the most recent changes to key-value pairs are valid.

The WAL is crucial for achieving high write throughput, as each PUT operation is recorded in the WAL. This mechanism ensures data persistence and integrity, allowing the LSM tree to maintain consistency and recover efficiently after a shutdown.

>MemTables 使用易失性内存意味着一旦断电或系统崩溃，数据就会丢失。为了解决这个问题，LSM 树会将 PUT 操作写入持久存储中的先写日志（WAL）。WAL 记录了一段时间内键值对的所有变化。当键值存储重新启动时，LSM 树会使用一个名为 WAL 恢复的过程来恢复其状态。WAL 恢复按顺序读取所有 WAL 数据，并将其重新应用到 MemTable。数据必须按顺序应用，因为只有键值对的最新更改才有效。
>
>WAL 对实现高写入吞吐量至关重要，因为每个 PUT 操作都会记录在 WAL 中。这种机制可确保数据的持久性和完整性，让 LSM 树保持一致性，并在关机后高效恢复。



However, frequent small-scale writes can reduce write throughput and create scalability bottlenecks. In multi-threaded environments, simultaneous WAL writes can cause lock contention, leading to spikes in write latency. A common solution is to merge small writes and perform bulk sequential writes, as seen in RocksDB's batch-group writes. This technique selects a leader thread among multiple worker threads with pending requests, collects all records, and writes them in a single batch. However, this can exacerbate tail latency issues, as waiting time for individual writes increases during batch collection and writing, particularly for larger records.

> （本段可删掉？）然而，频繁的小规模写入会降低写入吞吐量，造成可扩展性瓶颈。在多线程环境中，同时写入 WAL 会造成锁争用，导致写入延迟激增。一种常见的解决方案是合并小规模写入并执行批量顺序写入，如 RocksDB 的批量分组写入。这种技术会在多个有待处理请求的工作线程中选择一个领导线程，收集所有记录，然后以单个批次写入。不过，这可能会加剧尾部延迟问题，因为在批量收集和写入过程中，单个写入的等待时间会增加，尤其是对于较大的记录。

The introduction of Zoned Namespace Solid-State Drives (ZNS SSDs) offers several benefits, particularly for LSM-trees. By converting random writes into sequential writes, LSM-trees significantly boost write performance, aligning well with the architecture of ZNS SSDs. ZNS SSDs divide storage into zones where only sequential writes are allowed, simplifying the flash translation layer (FTL) and reducing garbage collection overhead.

Researchers have identified the potential suitability of ZNS SSDs as a backing store for LSM-trees due to the generation of sequential writes. However, despite these advantages, concurrent writes to the Write-Ahead Log (WAL) in ZNS environments can present performance bottlenecks.  

Therefore, we proposed A2WAL using the Zone append command and SPDK to solve the above problem

>分区命名空间固态硬盘（ZNS SSD）的推出带来了多种优势，尤其是对 LSM 树而言。通过将随机写入转换为顺序写入，LSM 树大大提高了写入性能，这与 ZNS 固态硬盘的架构非常吻合。ZNS 固态硬盘将存储划分为只允许顺序写入的区域，从而简化了闪存转换层（FTL）并减少了垃圾回收开销。
>
>研究人员发现，ZNS 固态硬盘由于可以生成顺序写入，因此可能适合作为 LSM 树的后备存储。然而，尽管有这些优势，在 ZNS 环境中并发写入前向日志（WAL）可能会造成性能瓶颈。
>
>因此，我们利用Zone append命令和 SPDK提出了A2WAL以解决上述问题

---

We developed a new WAL for ZNS SSDs called Append-aware WAL. This optimization is not limited to LSM-Tree storage engines. Due to their insufficient support for ZNS SSDs, our focus will be on LSM-tree storage engines on ZNS, using ZenFS as a reference. We will explain the modifications made to the existing WAL design for ZNS and discuss the integration of Zone-Friendly WAL in ZAFS within the context of RocksDB, comparing it with ZenFS.

Our Zone-Friendly WAL design is based on three key observations about WAL characteristics:

- \item  WALs are write-intensive and primarily issue small writes.
- \item  WALs are read only during database recovery.
- \item  WALs are usually small (i.e., 64 MiB).
  \end{itemize}

WALs' frequent write operations often lead to performance bottlenecks due to storage devices' limitations on small writes, while reads occur infrequently, mainly during recovery. Therefore, we prioritize write performance over read performance. Our goal is to enhance WAL write performance and concurrency by leveraging the peak performance of zone appends, thus eliminating bottlenecks from serial WAL writes and improving storage engine efficiency. To improve tail latency for LSM trees on ZNS SSDs, we utilize the zone append command, which allows the device to manage exact LBA locations and return the starting location to the host. This command enhances synchronization within ZNS SSDs, lifting host-side queue depth restrictions and significantly boosting write throughput and tail latency.

> 在 ZAFS 中，我们为 ZNS SSD 开发了一种新的 WAL，称为 Append-aware WAL。这种优化并不局限于 LSM-Tree 存储引擎。由于 LSM-Tree 存储引擎对 ZNS SSD 的支持不足，我们将以 ZenFS 为参考，重点关注 ZNS 上的 LSM-Tree 存储引擎。我们将解释针对 ZNS 的现有 WAL 设计所做的修改，并讨论在 RocksDB 的背景下将 Zone-Friendly WAL 集成到 ZAFS 中，并与 ZenFS 进行比较。
>
> 我们的Zone-Friendly WAL设计基于对WAL特性的三个关键观察：
>
> - WALs 是写密集型的，主要是发出小写。
> - 项目 WAL 在数据库恢复期间只能读取。
> - 项目 WAL 通常很小（即 64MB）。
...
> 由于存储设备对小规模写入的限制，WALs 频繁的写入操作往往会导致性能瓶颈，而读取则不常发生，主要是在恢复过程中。因此，我们优先考虑写入性能而非读取性能。我们的目标是通过利用区域附加的峰值性能来提高 WAL 写入性能和并发性，从而消除串行 WAL 写入的瓶颈并提高存储引擎的效率。为了改善 ZNS SSD 上 LSM 树的尾部延迟，我们使用了区域附加命令，该命令允许设备管理精确的 LBA 位置，并将起始位置返回给主机。该命令增强了 ZNS SSD 内部的同步性，解除了主机侧队列深度限制，显著提高了写入吞吐量和尾部延迟。

Building on these insights, we propose A2WAL (Append-aware  System), a novel key-value store WAL that enhances the write-ahead log using the zone append command for LSM trees on ZNS SSDs. We integrate Intel SPDK, a lightweight user-level NVMe driver, with RocksDB on ZenFS, establishing a robust baseline. 

Our contributions are summarized as follows:

1. A comprehensive evaluation of ZNS simulators and real ZNS devices.
2. The design and implementation of Append-aware WAL (A2-WAL) for ZNS SSDs.
3. Development of ZAFS, a ZNS-specific file system leveraging SPDK.
4. Extensive experimental validation demonstrating significant performance improvements over existing solutions.

This introduction sets the stage for a detailed exploration of ZNS SSDs and their integration with LSM-tree-based storage engines, highlighting the potential for substantial performance gains through the innovative use of zone append commands and advanced WAL techniques.



# 2. Background

### （**storage on flash based SSDs**）

Key-value (KV) stores are a fundamental storage model that organizes data into key-value pairs, offering fast data access and efficient management, especially for large datasets. Their simplicity and versatility make them easy to program and use, supporting various data types such as strings, numbers, and objects. Implementations include in-memory databases like Redis, known for high-speed access, and disk-based databases like LevelDB, optimized for efficient storage operations. Recent research focuses on enhancing performance, consistency, fault tolerance, compression, and security. The Log-Structured Merge-Tree (LSM-Tree) model is widely used in KV stores, addressing write bottlenecks by converting random writes into sequential writes. It features a hierarchical structure where data is written to an initial layer and then merged into subsequent layers, improving performance but introducing challenges like write amplification and read latency. Optimizations such as Bloom filters, tiered merge strategies, caching, and data compression mitigate these issues. Emerging technologies like persistent memory further enhance LSM-Tree models by offering larger capacities and improved performance. The success of the LSM-Tree model in write-intensive applications has driven its adoption in modern databases, exemplified by Google's LevelDB and RocksDB, leading to ongoing research and advancements to meet evolving data storage demands
...
Research on Zoned Namespace (ZNS) SSDs focuses on hardware optimization, file system enhancements, and application-specific adaptations. Leading manufacturers like Samsung and Western Digital have developed ZNS products such as ZNS+ and eZNS. File systems like F2FS and ZoneFS, as well as innovative uses in Linux, support ZNS. In applications, key-value stores, especially LSM-tree-based ones, benefit from ZNS due to their sequential write constraints, exemplified by RocksDB using ZenFS. ZNS+ introduces in-storage zone compaction for better performance, eZNS offers elastic partitioning to optimize resource allocation, and ZNSwap integrates a host-side garbage collector for efficient memory swapping. These advancements enhance storage performance and efficiency across various applications.
...
We have restructured ZenFS based on SPDK, introducing SPDK to bypass the Linux kernel and directly manage ZNS SSDs, which reduces latency. Additionally, we propose a zone-append-friendly WAL solution—ZoneWAL. Finally, we implemented a lifecycle-based zone grouping allocation strategy to optimize ZenFS space utilization.
...
Write-Ahead Logging (WAL) is a technique in computer science used to ensure atomicity and durability in database systems. It involves logging all modifications before applying them, providing a mechanism for crash and transaction recovery. Modern file systems often use variants of WAL, such as journaling, for metadata updates.

In LSM-Tree-based storage engines, WAL is used specifically as follows: When writing to the volatile MemTable, data cannot be preserved if a power failure occurs. To address this consistency issue caused by power outages, storage engines maintain a WAL on persistent storage. First, the records from the put (write) request are stored in the WAL, and then written to the MemTable in memory. During data recovery, all WAL data is read sequentially, and the corresponding operations are executed in order to restore the MemTable. Since only the latest changes to key-value pairs are valid in LSM-Trees, WAL records must be executed in sequence.

However, this design can lead to frequent small write operations, creating a performance bottleneck. Thus, WAL is crucial for achieving high write throughput. On one hand, since LSM trees are typically deployed on fast and highly parallel NVMe flash SSDs, internal management operations issued by SSDs compete with LSM trees for storage resources, leading to unstable throughput. On the other hand, lock contention in multi-threaded environments results in write latency.

One popular solution to these issues is batch writing, as implemented in RocksDB. This process dynamically selects one leader thread among multiple worker threads with pending put requests, collects all remaining records, and lets the leader thread write them on behalf of the others. However, this technique does not improve and may even exacerbate tail latency issues. During the collection and writing of these records, all records in the batch share the write time, potentially increasing the wait time for a single write, especially if the record is large.

ZNS improves LSM tree throughput and makes latency more stable but poses significant write throughput challenges for LSM tree components like WAL. The main reason is ZNS's strict prohibition of random writes and simultaneous write I/Os to the same zone by applications. Therefore, PUT operations to the WAL are serialized, allowing only one PUT to be processed at a time, limiting WAL throughput. Moreover, the tail latency effect inherent to batch writing solutions reduces the latency benefits brought by ZNS. Consequently, ZNS does not exhibit its expected performance advantages.

### Zone Append Command

To address write I/O throughput limitations, ZNS introduces a zone append operation. The Zone Append command allows the host system to append data to the end of a specified zone without performing an erase operation beforehand. Unlike traditional random writes, which usually require erasing flash cells before writing new data, the Zone Append command does not specify the LBA in the I/O submission request. Instead, the SSD determines the LBA during processing and returns the address in the response. In other words, the Zone Append command is typically executed asynchronously, allowing the application to continue other operations immediately after sending the append request without waiting for the data to be written.

Therefore, the Zone Append command offers excellent concurrency, making it a suitable alternative for WAL, effectively enhancing the throughput of sequential small file writes. This command shows promise in resolving the write bottleneck for highly concurrent small files (e.g., WAL). However, a challenge remains: since the append operation returns the address in the response, the actual write order of concurrent write commands is determined by the device, which may not match the order in which the commands were issued, resulting in out-of-order storage. In case of a system crash, reading from the WAL for data recovery yields invalid out-of-order results. We address this issue by adding identifiers to each append operation and employing a novel recovery technique to handle WAL out-of-order writes.
...
There are four main operations in WAL: **write, recovery, allocation, and deletion**.

> Diagram of Zone-Friendly WAL's field structure

### WAL Write

In ZenFS, during WAL write operations, the data from a PUT request is appended to the end of the WAL, triggering I/O operations to the zone write pointer. If another PUT operation is issued simultaneously, it must wait for the previous PUT to complete. In ZNFS, to increase concurrency, A2WA's issue zone appends without waiting for the append to complete. This allows multiple PUT operations to be written to the WAL concurrently, but concurrent zone appends lead to out-of-order writes, which cannot guarantee the order of PUT requests.

**WAL Header** Therefore, Zone-Friendly WAL helps reorder writes by adding a 128-bit header before each WAL write. We combine the WAL data and the header (in yellow) into a WAL entry. The header consists of a 64-bit sequence number and the size of the WAL entry. The sequence number increments atomically, representing the absolute data order. Each WAL maintains its sequence number to avoid overflow risks (WALs are unlikely to reach 2^64 entries). The entry size is used to infer the position of subsequent WAL entries. Why store the entry size? If using the address returned by the zone append to determine the data storage position, this address is volatile and would require another write operation to store it in memory.

**WAL Buffer** Since the optimal request size for appends may differ from the page size and KV pairs can be significantly smaller than the page size, A2WAL  allows buffering (similar to ZenFS). During WAL writes, the data is first copied to a buffer. Once the buffer is full or the WAL is synchronized (e.g., fsync, close), we use zone append to write the data to the SSD.

### WAL Recovery

During WAL recovery, the LSM tree sequentially scans its WAL and applies the read data to its memory table. The LSM tree reads a few KB of data at a time. However, A2WAL stores data on the SSD out of order; A2WAL needs to restore its original order by creating a mapping from logical addresses (i.e., offsets) to physical addresses.

When reading, it first finds the WAL entry corresponding to the logical address using the information in the WAL header. Since the sequence number increases monotonically and data is appended to the WAL, WAL entries with higher sequence numbers have strictly higher logical addresses. Specifically, the logical address of a WAL entry with sequence number x equals the logical address of the WAL entry with sequence number x-1 plus its size. For example, the PUT stored at sequence number 1 is located at the logical address of PUT 0 plus PUT 0's size. When reading from the WAL, A2WAL reads the entry at the corresponding logical address.

However, retrieving the WAL header is challenging because the size of each WAL entry may vary, preventing A2WAL from pre-determining the position of the WAL header, except for the first request. Thus, A2WAL  needs to sequentially scan the entire WAL to locate the WAL header entry by entry. An alternative solution is to read the WAL once and keep a logical address mapping in memory, but this requires memory proportional to the WAL size. Therefore, A2WAL adopts a more efficient approach: using barriers. A2WAL inserts barriers at predefined page intervals (called Pbarriers). At the barrier, A2WAL synchronizes all zone append operations (i.e., waits for all operations to complete). Requests are sorted between consecutive barriers, meaning the physical address of appends after a barrier is strictly higher than those before the barrier. During reads, A2WAL first finds the nearest barrier and then reads all data between the two barriers. A2WAL  then creates a mapping of WAL entries based on the sequence numbers and sorts the mapping by sequence numbers. This design limits I/O and memory usage to a configurable upper limit, allowing A2WAL to maintain the mapping of one page interval at a time. Furthermore, the mapping can be cached since reads are sequential and subsequent reads are likely within the same barrier. The expected recovery cost of A2WAL  is slightly higher than traditional WAL and is proportional to the barrier size (sorting is O(n log n)).

### WAL Allocation and Deletion

In ZenFS, WAL allocation involves assigning a zone to the WAL. This zone is not dedicated solely to the WAL and can be shared with other LSM-tree components. However, A2WAL imposes stricter requirements, necessitating dedicated zones for the WAL.

This requirement arises for two reasons: first, to prevent other LSM-tree components from issuing write requests to the same zone as the zone appends, which would necessitate the zone append operation to wait for the write to complete, negating its advantages. Second, A2WAL ’s recovery process demands that all WAL data be stored consecutively, ensuring data continuity.

Regarding WAL deletion, both ZenFS and A2WAL treat it as a storage resource release operation, similar to other data deletions. Since A2WAL uses dedicated zones, specific zones marked for deletion can be safely reset at any time without impacting other data. Deletion can be either proactive (i.e., immediate execution) or delayed (i.e., executed when storage resources need to be freed).



In summary, ZAFS architecture optimizes the use of ZNS SSDs by incorporating SPDK to bypass the Linux kernel, thereby reducing latency. The introduction of the Zone-Friendly WAL (ZoneWAL) addresses the challenges of write-intensive operations and improves concurrency by utilizing zone append commands effectively. The lifecycle-based zone grouping allocation strategy further enhances ZenFS space utilization, resulting in improved overall performance.

ZoneWAL, with its innovative approach to handling write operations, buffer management, and recovery mechanisms, demonstrates significant improvements in handling small, frequent writes, which are typical in LSM-tree storage engines. By leveraging zone append commands and introducing efficient recovery techniques, ZAFS ensures high write throughput and stable latency, making it a robust solution for modern storage systems.
...
实验平台使用配有双路CPU，大容量DDR4内存的高性能服务器，并装有两块浪潮Inspur NS8600G2-Z ZNS SSD，其具体配置如表4-1所示。
...
| 内存     | 503GiB                                        |
| SSD      | Inspur NS8600G2-Z                             |
| 操作系统 | Ubuntu  22.04.3 LTS（Linux 5.15.0）           |

### 

### 4.2 模拟器VS真实设备

ZNS SSD中，有一些重要参数在生产时确定，是不可更改的，这些参数与内部结构高度相关。例如Zone Size，目前，在区域大小的设置上有很多争议，西数设计的区域大小在GB级，而三星设计的 Small-Zone ZNS的区域大小只有96MB。像这样的参数对比只能通过模拟器来测试。

实验中使用FIO-3.28，该版本已支持ZNS的测试。

模拟平台使用NVMeVirt实现，模拟参数参考WD_ZN540。
...
写入负载：块大小为64KB，16个并发线程，每个线程最多只拥有1个Zone。对比结果如表4-2所示。

表4-2 NVMeVirt模拟器评估

|                    | 写入速度 | 平均延迟 |
| ------------------ | -------- | -------- |
| NVMeVirt           | 2626MB/s | 75us     |
| Inspur  NS8600G2-Z | 5315MB/s | 153us    |

 

**模拟器的误差较大：**根据上述实验结果可以观察到，模拟器与真实硬盘的差距较大。通过阅读NVMeVirt原文，发现其支持模拟的最大NVMe link 带宽达不到PCIe4.0水平，而我们使用的物理设备是PCIe4.0 SSD，这是二者写入速度差距较大的直接原因在延迟方面，由于该模拟器以内核模块的形式实现，相较于其他模拟器没有虚拟机开销，但是，由于它的数据只写入到内存中，相较于真实设备，延迟过低，且只能通过设定一个固定值来弥补这个问题，这导致其对延迟的模拟非常局限。我们测试了在其他参数配置下模拟器的性能表现，均存在一倍以上的差距。因此，在硬件基准性能测试中二者差距较大，可见模拟器在性能上往往不能精确反映设备的真实情况。

### 1.1.2   RocksDB + ZenFS应用测试

基于RocksDB + ZenFS分别对模拟和物理环境进行测试，记录吞吐量随时间的变化。

​                               

图4-1 ZNS模拟器与ZNS设备的吞吐量波动情况对比

从上图中可以观察到，尽管二者数值上差距较大，但是NVMeVirt的吞吐量随时间变化的趋势与真实设备基本一致，该模拟器能较好地观察设备的性能波动情况，有助于分析程序中的性能瓶颈。
...
在实际应用中，会出现某类型的数据仅写入单个指定Zone的情况（例如上文分析的RocksDB中的WAL文件），因此，测试不同情况下单个Zone的性能很有意义。分别使用三种I/O引擎，在不同的块大小下进行单Zone写入测试，结果如图4-2所示。

![image-20240719194341929](%E6%AF%95%E8%AE%BE_NAS_en.assets/image-20240719194341929.png)

写入随着每次请求的块大小增大而增大，单个Zone的写入速度最大达到2500MiB/s。这与全盘的写入速度差距较大，因为只使用一个Zone不能达到该盘最大带宽。





![image-20240719194352052](%E6%AF%95%E8%AE%BE_NAS_en.assets/image-20240719194352052.png)



在不同的块大小下三种I/O路径的单Zone写入吞吐量结果如图4-3所示。观察到单个Zone的IOPS在4K时最大。并且吞吐量随着每次请求的块大小增大而下降。
...
为了评估不同块大小和线程数对写入速度和IOPS的影响，我们设计并实施了一系列测试。测试配置包括使io_uring作为I/O引擎，并在五种不同的块大小（4k、16k、32k、64k、128k、256k）和五个不同的线程数（1、2、4、8、16）下进行测试。

![image-20240719194358292](%E6%AF%95%E8%AE%BE_NAS_en.assets/image-20240719194358292.png)

如图4-4所示，总体而言，写入带宽随块大小和线程数的增加而增加，符合预期中的结果。较小的块大小（如4k）导致显著较低的带宽，这是由于处理更多I/O操作带来的额外开销。在块大小从16k逐渐增加到256k的过程中，带宽不断提升，但增长率逐渐减小，直至达到硬盘最大写入速度5300MiB/s。线程数对带宽的影响在不同块大小下有所不同。在较小的块大小下（如4k），增加线程数能够显著提高带宽，说明多线程能够更有效地利用I/O子系统的并行性。而在较大的块大小下（如256k），线程数的增加对带宽的提升相对有限，因为此时磁盘带宽已经接近饱和，多线程无法进一步显著提高性能。因此，实验结果表明，多线程和大块大小均有助于提升带宽，但在一定块大小和线程数之后，带宽的增长会受到硬件和系统瓶颈的限制。

![image-20240719194406215](%E6%AF%95%E8%AE%BE_NAS_en.assets/image-20240719194406215.png)

图4-5中展示了不同块大小和线程数对吞吐量的影响。结果显示，4k块大小下，IOPS随线程数显著提高，从单线程的约100 KIOPS增加至16线程的约800 KIOPS，表明在较小块大小下，高并发度显著提升I/O子系统的并行性和性能。16k块大小下，IOPS亦随线程数增加，从约100 KIOPS提高至350 KIOPS，但提升幅度相较于4k块大小有所减小。随着块大小进一步增加至32k、64k、128k和256k，IOPS提升幅度逐渐减小，32k块大小下从约50 KIOPS提高至150 KIOPS，64k块大小下从约30 KIOPS提高至90 KIOPS，128k块大小下从约10 KIOPS提高至40 KIOPS，256k块大小下从约5 KIOPS提高至20 KIOPS。这表明在较大块大小下，多线程对性能提升的作用逐渐减弱，甚至达到瓶颈。因此，在需要高IOPS的应用场景中，选择较小的块大小和多线程配置可以显著提升性能，而在块大小较大的场景中，多线程对性能提升的效果有限。
...
![image-20240513033213069](%E6%AF%95%E8%AE%BE_en.assets/image-20240513033213069.png)

> 
>
> linux I/O栈 结构图
>
> ![img](%E6%AF%95%E8%AE%BE_en.assets/Linux-storage-stack-diagram_v6.2.png)
>
> LinuxIO栈的复杂程度
...
![在这里插入图片描述](%E6%AF%95%E8%AE%BE_en.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1pfU3RhbmQ=,size_16,color_FFFFFF,t_70.png)
...
![image-20240513033322894](%E6%AF%95%E8%AE%BE_en.assets/image-20240513033322894.png)
...
![image-20240508205932120](%E6%AF%95%E8%AE%BE_en.assets/image-20240508205932120.png)
...
![ZenFS架构图](%E6%AF%95%E8%AE%BE_en.assets/ZenFS%E6%9E%B6%E6%9E%84%E5%9B%BE.png)
