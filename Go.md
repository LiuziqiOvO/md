...
- 使用 `go build` 命令来编译。包括依赖一起生成可执行文件。
...
- 使用 `go install` 编译并安装程序，将可执行文件放到 `$GOPATH/bin` 或 `$GOBIN`。

**运行你的程序**

- 只在命令行中运行编译后的可执行文件或者使用 `go run` 命令直接运行源代码。
...
- **测试文件**通常位于与被测试的源代码相同的包中，文件名遵循 `xxx_test.go` 的模式。
- **测试函数**以 `Test` 开头，接受一个 `*testing.T` 类型的参数。
