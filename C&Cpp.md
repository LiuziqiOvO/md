...
`std::unordered_set` 是 C++ 标准库中提供的一个无序集合容器，基于哈希表实现。它存储唯一的元素，元素的顺序是无序的。以下是 `std::unordered_set` 的常用方法及其功能：

### **构造函数**

```c++
std::unordered_set<int> s1; // 空集合
std::unordered_set<int> s2 = {1, 2, 3}; // 初始化列表
std::unordered_set<int> s3(s2.begin(), s2.end()); // 从另一个集合初始化
```
...
- `emplace(value)`
  原地构造并插入一个元素（比 `insert` 更高效）。
- `erase(value)`
  删除指定值的元素，返回删除的元素个数（0 或 1）。
- `erase(iterator)`
  删除指定位置的元素。
- `clear()`
  清空集合中的所有元素。
