...
由于逐层压缩，第0层、第1层和第2层的SSTables有更多的机会被压缩，这导致这些较低层级的SSTables频繁更新，其寿命通常短于较高层级的SSTables。大多数写入和更新集中在低层级的SSTables，导致极不平衡的写分布，这会显著缩短SSD的有限寿命。
...
Figure 2展示了ZNS SSD的典型架构。多个位于不同闪存芯片上的块首先被分组成一个区域 [20]。这种方式下，ZNS SSD的逻辑地址空间被划分为固定大小的（例如512MB）区域，这些区域可以随机读取但必须顺序写入。
...
NVMe ZNS规范中未定义ZNS SSD的映射方案，由供应商自行实现区域映射以满足特定的目的。通常，映射机制可以分为静态映射和动态映射两种类型。

静态映射方案中，一个区域被静态映射到多个固定的擦除块，从而减少或甚至消除了区域到块映射表的空间开销以及地址转换的时间开销 。此外，每个区域的内部结构（例如区域之间的干扰）对主机更加透明，使得能够更轻松地与应用程序的访问特性进行协同优化。静态映射的访问性能更加可预测。

动态映射方案展现出更高的灵活性，允许驱动器供应商在打开区域时动态地将特定块映射到区域 。然而，与静态映射方案相比，动态映射方案会引入更多的空间和时间开销。此外，考虑到ZNS SSD内区域与资源之间的动态关系，设计既能利用应用程序访问模式又能充分发挥ZNS SSD特性的协同优化变得更加复杂和难以捉摸。
...
3. **ZNS SSD的磨损均衡**：
   - 现有研究主要关注设备内的磨损均衡技术，分为两类：
     - 将空闲擦除块轮询地映射到新分区 。
     - 优先将擦除次数最少的块映射到新分区 。
   - 这些技术被总结为动态磨损均衡，可以有效地实现空闲块的磨损均衡。
   - 然而，持有冷数据的打开分区上的块不能立即回收，导致擦除次数在分区间分布不均。
   - 因此，本文提出了一种新的面向磨损的分区分配器，以改进ZNS SSD的寿命。

#### 分区重置机制（Zone Reset）

ZNS SSD将分区回收的责任转移到主机，通常在文件系统中实现，如ZenFS中的分区清理和F2FS中的段清理。

1. **当前的分区重置机制**：
   - 当空闲空间低于指定阈值时触发分区回收。
   - 典型流程是选择一个有效数据较少的受害分区，将其上所有有效数据迁移到其他分区，然后重置受害分区以释放空间。
   - 这可能导致大量数据迁移开销，造成性能下降和I/O阻塞 。
2. **运行时分区重置方案**：
   - 现代分区文件系统（如ZenFS）中实现了一种运行时分区重置方案，其核心思想是在分区上所有数据失效时重置分区。
   - 这种方案可以有效减少分区回收调用的频率。
   - 然而，即使分区有大量未使用空间，只要所有数据失效，该分区仍会被重置。
   - 分区重置命令会擦除分区上的所有块，包括未使用块，这些不必要的擦除操作进一步减少了SSD寿命。
   - 为此，本文提出了一种基于部分擦除的分区重置方法，以减少对未使用块的不必要擦除操作。

通过这些技术，ZNS SSD可以有效地应对写密集型工作负载中的挑战，延长设备寿命并优化性能。
